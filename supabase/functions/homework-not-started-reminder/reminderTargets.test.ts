...
describe('selectReminderTargets', () => {
  it('알림 시각이 지났고 시작한 게 없으면 대상이다', () => {
    const targets = select({ now: '21:00', homeworkItems: [item({}), item({ id: 'h2' })] });
    expect(targets).toEqual([{ studentId: 's1', remindAt: '21:00', homeworkCount: 2 }]);
  });

  it('알림 시각 전에는 대상이 아니다', () => {
    expect(select({ now: '20:45', homeworkItems: [item({})] })).toEqual([]);
  });

  it('설정 행이 없으면 기본 시각을 쓴다', () => {
    expect(select({ now: '20:59', homeworkItems: [item({})] })).toEqual([]);
    expect(select({ now: '21:01', homeworkItems: [item({})] })).toHaveLength(1);
  });
  ...
  it('한 학생이 시작했어도 다른 학생은 따로 판정한다', () => {
    ...
    const targets = select({ now: '21:00', homeworkItems: items, startedItemIds: ['h1'] });
