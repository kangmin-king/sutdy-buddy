...
> **대체됨(2026-08-27).** 이 스펙은 차단할 앱 목록을 유지한 채 이탈 감지 신호만 분리하는
> 설계였다. 검토 중 방향이 바뀌어 차단 목록을 허용 목록으로 뒤집기로 했고, 그 결과 이탈
> 감지가 필요 없어져 아래 설계의 대부분이 무효가 되었다. 유효한 설계는
> [2026-08-27-distraction-stop-allowlist-design.md](2026-08-27-distraction-stop-allowlist-design.md)
> 이며, 이 문서는 배경과 문제 진단 기록으로만 남긴다.
